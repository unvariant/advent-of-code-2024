...
static mut RULES2: Rules = Rules([[0; 32]; 100]);
static mut PTRS2: [*mut u32; 100] = unsafe {
    [
...
static mut SCRATCH2: [u32; 512] = [0; 512];
static mut INDEXES2: [u32; 100 * 512] = [0x7fffffff; 100 * 512];
        ...
        if pos != 32 {
            ...
            if !valid {
                for i in 0..n {
                    let page = pages.add(i as usize).read();
                    let curr = (&RULES2.0[page as usize]).as_ptr() as *const u32x8;
                    let empty_rule = u32x8::splat(0x7fffffff);
                    let a: u32x8 = _mm256_i32gather_epi32::<4>(
                        indexes as *const i32,
                        curr.add(0).read_unaligned().into(),
                    )
