...
unsafe fn cross(s: &[u8]) -> u32 {
    ...
    loop {
        ...
        let pos0 =
            (hash0.simd_eq(u8x32::splat(b'M' + b'S')) & r110.simd_eq(u8x32::splat(b'A'))).to_int();
        let pos1 =
            (hash1.simd_eq(u8x32::splat(b'M' + b'S')) & r110.simd_eq(u8x32::splat(b'A'))).to_int();

        let pos2 =
            (hash2.simd_eq(u8x32::splat(b'M' + b'S')) & r111.simd_eq(u8x32::splat(b'A'))).to_int();
        let pos3 =
            (hash3.simd_eq(u8x32::splat(b'M' + b'S')) & r111.simd_eq(u8x32::splat(b'A'))).to_int();
